...
internal class SimpleSimuleringServiceTest {
    ...
    fun shouldReturnStatusAndFeilkodeWhenDuplicateOpptjeningsperiodeEndDate() {
        ...
        val asyncResponse = AsyncExecutor.AsyncResponse<TPOrdning, TpLeverandor>().apply {
        ...
        Mockito.`when`(opptjeningsperiodeService.getLatestFromOpptjeningsperiode(anyNonNull()))
                .thenThrow(DuplicateOpptjeningsperiodeEndDateException("exception"))
    ...
    fun `Should add response info when no opptjeningsperiode available`() {
        Mockito.`when`(asyncExecutor.executeAsync<TPOrdning>(anyNonNull())).thenReturn(AsyncExecutor.AsyncResponse())
        Mockito.`when`(opptjeningsperiodeService.getOpptjeningsperiodeListe(anyNonNull(), anyNonNull()))
    ...
    fun `Should return response object`() {
        ...
        Mockito.`when`(tpRegisterConsumer.getTpOrdningerForPerson(anyNonNull())).thenReturn(listOf(tpOrdning))
        Mockito.`when`(tpLeverandorConfig.tpLeverandorList()).thenReturn(listOf(tpLeverandor))
        Mockito.`when`(asyncExecutor.executeAsync<TPOrdning>(anyNonNull()))
                .thenReturn(AsyncExecutor.AsyncResponse<TPOrdning, TpLeverandor>().apply { this.resultMap[tpOrdning] = tpLeverandor })
        Mockito.`when`(simuleringEnpointRouter.simulerPensjon(anyNonNull(), anyNonNull(), anyNonNull()))
                .thenReturn(s1)
        Mockito.`when`(opptjeningsperiodeService.getLatestFromOpptjeningsperiode(anyNonNull()))
    ...
    fun `Should increment metrics`() {
        ...
        Mockito.`when`(tpRegisterConsumer.getTpOrdningerForPerson(anyNonNull())).thenReturn(listOf(tpOrdning))
        Mockito.`when`(tpLeverandorConfig.tpLeverandorList()).thenReturn(listOf(tpLeverandor))
        Mockito.`when`(asyncExecutor.executeAsync<TPOrdning>(anyNonNull())).thenReturn(AsyncExecutor.AsyncResponse<TPOrdning, TpLeverandor>().apply { this.resultMap[tpOrdning] = tpLeverandor })
        Mockito.`when`(opptjeningsperiodeService.getLatestFromOpptjeningsperiode(anyNonNull()))
                .thenReturn(tpOrdning)
        Mockito.`when`(simuleringEnpointRouter.simulerPensjon(anyNonNull(), anyNonNull(), anyNonNull()))
                .thenReturn(s1)
