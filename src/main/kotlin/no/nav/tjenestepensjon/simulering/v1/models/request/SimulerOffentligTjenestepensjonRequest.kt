...
data class SimulerOffentligTjenestepensjonRequest(
    ...
    constructor(
        ...
        antallArInntektEtterHeltUttak = findAntallArInntektEtterHeltUttak(
            simulerPensjonRequest.inntekter, heltUttak.datoFom
        ),
    ...
    constructor(
        ...
        antallArInntektEtterHeltUttak = findAntallArInntektEtterHeltUttak(
            simulerPensjonRequest.inntekter, forsteUttak.datoFom
        ),
        tpForholdListe = tpOrdningStillingsprosentMap.map { TpForhold(it.key, it.value) })
    ...
    companion object {
        ...
        fun findInntektForUttak(inntektList: List<Inntekt>, uttaksDato: LocalDate) =
            findInntektOnDate(inntektList, uttaksDato)?.let {
                inntektList.sorted().reversed().dropWhile { inntekt: Inntekt -> it != inntekt }.getOrNull(1)
            }?.inntekt?.toInt()
    }
}
